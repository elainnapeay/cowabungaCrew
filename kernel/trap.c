...
cowfault(pagetable_t pagetable, uint64 va)
{
   ...
   pte_t *pte = walk(pagetable, va, 0); //find page table entry
   //case for not finding pte
   if(pte == 0)
   {
      return -1;
   }
   ...
   //if either flag is clear: illegal address
   if((*pte & PTE_U) == 0 || (*pte & PTE_V) == 0)
   {
      return -1;
   }
  //  if ((*pte & (PAGE_FREE | PAGE_UNALLOCATED)) != 0) {
  //       return -1; // Page is marked as free or unallocated
  //  }
   //copy page
   ...
   //map new page into address space
